...
if __name__=="__main__":
    for x in os.walk("./11032025/sine_sine/"):
        for y in x[2]:
            PATH    = f"{x[0]}/{y}"
            PATHTarget = PATH.replace("./11032025/sine_sine/","./exported/")
            # PATH    = os.path.expanduser("modelPreFK_0_8.pth")
            net = DNNSinogramSR(Sine(),Sine()).to(device)
            net.load_state_dict(torch.load(PATH, weights_only=True,map_location=device))
            net.eval()

            traced_model = torch.jit.script(net)
            traced_model.save(PATHTarget[:-1])
